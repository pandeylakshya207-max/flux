...
from flux.nn.conv import Conv2D, Flatten
...
Xtr, ytr = load_batch(os.path.join(base, "data_batch_1"))
...
class SimpleCNN:
    def __init__(self):
        self.c1   = Conv2D(3,  16, 3, padding=1)
    ...
    def __call__(self, x):
        ...
        return self.fc(x)
    def parameters(self):
        return self.c1.parameters() + self.c2.parameters() + self.fc.parameters()
    def zero_grad(self):
...
print("Training (10k subset, CPU)...")
for epoch in range(1, 11):
    losses = []
    for Xb, yb in get_batches(Xtr, ytr, 64):
        logits = model(Xb)
    ...
    print("Epoch", epoch, "- loss:", round(float(np.mean(losses)),4), " test_acc:", round(acc*100,2), "%")

final = accuracy(Xte, yte)
print("Final test accuracy:", round(final*100, 2), "% (10k train subset, CPU)")
